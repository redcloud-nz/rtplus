...
export default async function SkillPage(props: { params: Promise<{ 'skill-id': string }>}) {
    const { 'skill-id': skillId } = await props.params
    if(!validateUUID(skillId)) throw new Error(`Invalid skill-id (${skillId}) in path`)
                ...
                <Card>
                    ...
                    <CardContent>
                        <DL>
                            ...
                            <DLDetails>
                                <Link href={Paths.system.skillPackages.skillPackage(skill.skillPackage.id).index}>{skill.skillPackage.name}</Link>
                            </DLDetails>
