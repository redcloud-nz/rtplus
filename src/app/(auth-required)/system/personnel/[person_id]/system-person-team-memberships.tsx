 ...
 */
export function System_Person_TeamMemberships_Card({ personId }: { personId: string }) {
    const trpc = useTRPC()
    ...
    const columns = useMemo(() => defineColumns<TeamMembershipData & { team: TeamData }>(columnHelper => [
        ...
        columnHelper.accessor('team.name', {
            ...
            cell: ctx => (match(ctx.row.getEditMode())
                ...
                .otherwise(() => 
                    <TextLink href={Paths.system.person(personId).teamMembership(ctx.row.original.teamId).href}>{ctx.getValue()}</TextLink>
                )
            ),
            enableGrouping: false,
            enableHiding: false
