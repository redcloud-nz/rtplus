...
export default function CompetencyRecorder_Session_Assessees_Page() {
    ...
    const { data: assignedAssessees } = useSuspenseQuery(trpc.skillCheckSessions.getAssignedAssessees.queryOptions({ sessionId: session.sessionId }))
    ...
    const mutation = useMutation(trpc.skillCheckSessions.updateAssessees.mutationOptions({
        async onMutate({ additions, removals }) {
            await queryClient.cancelQueries(trpc.skillCheckSessions.getAssignedAssessees.queryFilter({ sessionId: session.sessionId }))

            const previousData = queryClient.getQueryData(trpc.skillCheckSessions.getAssignedAssessees.queryKey({ sessionId: session.sessionId }))
            queryClient.setQueryData(trpc.skillCheckSessions.getAssignedAssessees.queryKey({ sessionId: session.sessionId }), (old = []) => 
        ...
        onError(error, _data, context) {
            queryClient.setQueryData(trpc.skillCheckSessions.getAssignedAssessees.queryKey({ sessionId: session.sessionId }), context?.previousData)
            toast({
        ...
        onSettled() {
            queryClient.invalidateQueries(trpc.skillCheckSessions.getAssignedAssessees.queryFilter({ sessionId: session.sessionId }))
        }
    }))
