...
function defineSubApps(teamSlug: string): { title: string, href: string, icon?: typeof ClockIcon, iconForeground: string, iconBackground: string, description: string }[] {
    const teamPath = Paths.team(teamSlug)
    return [
        {
        title: 'Availability',
        href: Paths.team(teamSlug).availability,
        icon: ClockIcon,
        ...
        <AppPage label={sessionClaims.org_name}>
            <div className="container mx-auto">
                ...
                <ul role="list" className="container mx-auto grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    {defineSubApps(teamSlug).map((action, actionIdx) => 
