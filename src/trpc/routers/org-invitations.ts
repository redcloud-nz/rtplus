...
import { orgInvitationFormSchema } from '@/lib/forms/org-invitation'

import { createTRPCRouter, teamAdminProcedure } from '../init'
import { OrgInvitationBasic } from '../types'
...
export const orgInvitationsRouter = createTRPCRouter({
    byCurrentTeam: teamAdminProcedure
        .query(async ({ ctx }): Promise<OrgInvitationBasic[]> => {
    ...
    create: teamAdminProcedure
        ...
        .mutation(async ({ ctx, input }): Promise<OrgInvitationBasic> => {
...
function toOrgInviteBasic(invite: OrganizationInvitation): OrgInvitationBasic {
    return {
        id: invite.id,
